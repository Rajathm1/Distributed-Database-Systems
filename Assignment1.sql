...
SELECT * FROM ratings;
--Query 2
SELECT * FROM ratings WHERE movieid<316;
--Query 3
SELECT * FROM ratings WHERE rating>2.5;
--Query 4
SELECT * FROM ratings WHERE movieid>200 and rating<3;
--Query 5
SELECT * FROM ratings WHERE movieid<350 and rating>4;
...
DROP TABLE IF EXISTS f1;
DROP TABLE IF EXISTS f2;
CREATE TABLE f1 AS SELECT movieid, rating FROM ratings;
CREATE TABLE f2 AS SELECT userid FROM ratings;

/* Write your explanation as a comment */
/* Performed a vertical fragmentation
   f1: Contains movieid and rating 
   f2: Contains userid
   ...
   only if each data item in R can also be found in some Ri
		All movieid and rating columns data items are present in f1
	...
	Disjointness: If relation R is decomposed into fragments R1, R2, ..., Rn, and data item di is in Rj, 
	then di should not be in any other fragment Rk (k ≠ j ).
		There are no common columns in f1 and f2, so no data item of f1 is present in f2. 
...
-- rating are from 0 to 5 with incrementing value with 0.5
DROP TABLE IF EXISTS f1;
DROP TABLE IF EXISTS f2;
DROP TABLE IF EXISTS f3;
CREATE TABLE f1 AS SELECT * FROM ratings WHERE rating BETWEEN 0 AND 2;
CREATE TABLE f2 AS SELECT * FROM ratings WHERE rating BETWEEN 3.5 AND 4;
...
SELECT userid, movieid FROM f1 WHERE rating BETWEEN 4 AND 5;
